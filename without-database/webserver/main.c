...
#ifdef DBG_PRINT
        fprintf(stdout, "+ read holding:%d length:%d\n", addr, count);
#endif
...
void modbus_read_input_by_list(duda_request_t *dr, modbus_t *ctx, uint16_t *list, int list_len)
{
    ...
    for (int i = 0; i < list_len; i++) {
        ...
        uint16_t count = list[i * 2 + 1];
#ifdef DBG_PRINT
        fprintf(stdout, "+ read input:%d length:%d\n", addr, count);
#endif
        if (reg_count + count > MAX_COUNT)
            break;
        if (simulate) {
...
#ifdef MSG_PRINT
    fprintf(stdout, "+ modbus=%s reg=%s range=%s data=%s\n",
        modbus == NULL ? "(0)": modbus,
        req == NULL ? "": req,
        range == NULL ? "": range,
        data == NULL ? "": data);
#endif
...
int duda_main()
{
    ...
    if (simulate) {
        ...
        ctx0 = modbus_new_rtu("/dev/ttyUSB0", MODBUS_BAUD_RATE, 'N', 8, 1);
        if (modbus_connect(ctx0) == -1) {
            ...
            modbus_set_slave(ctx0, MODBUS_SLAVE_ADDR);
			modbus_rtu_set_serial_mode(ctx0, MODBUS_RTU_RS485);
			modbus_rtu_set_rts(ctx0, MODBUS_RTU_RTS_DOWN);
            fprintf(stdout, "+ ttyUSB0 connection active  slave addr:%d  baud:%d\n",
                    MODBUS_SLAVE_ADDR, MODBUS_BAUD_RATE);
        };

        // Set up a Modbus connection to CIU 1.
        ctx1 = modbus_new_rtu("/dev/ttyUSB1", MODBUS_BAUD_RATE, 'N', 8, 1);
        if (modbus_connect(ctx1) == -1) {
            ...
            modbus_set_slave(ctx1, MODBUS_SLAVE_ADDR);
			modbus_rtu_set_serial_mode(ctx1, MODBUS_RTU_RS485);
			modbus_rtu_set_rts(ctx1, MODBUS_RTU_RTS_DOWN);
            fprintf(stdout, "+ ttyUSB1 connection active  slave addr:%d  baud:%d\n",
                    MODBUS_SLAVE_ADDR, MODBUS_BAUD_RATE);
        };
    };
