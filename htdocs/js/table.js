...
app.controller('tableCtrl_m1', ["$rootScope", "$scope", "$http", "$timeout", "$location", "Idle", "Keepalive", "$uibModal", "$cookieStore", "$window",  function ($rootScope, $scope, $http,  $timeout, $location, Idle, Keepalive, $uibModal, $cookieStore, $window) {
    ...
    $scope.show_title = function (title, dataFmtd, Address) {
        $scope.show_title.show = true;
        $scope.title = title;
        $scope.dataFmtd = dataFmtd;
        $scope.Address = Address;
        $scope.data_Loading1 = false;
        $rootScope.$emit("Address", $scope.Address);
        //$scope.$broadcast('Address', $scope.Address);
    };
...
app.controller('Database_chart', ["$rootScope", "$scope", "$http", "$timeout", function ($rootScope, $scope, $http, $timeout) {
    $scope.currentPage = 1;
    $scope.pageSize = 7;
    FusionCharts.ready(function(){
        ...
        "events": {
            "initialized": function(e) {
                var flag = 0;
                $rootScope.$on("Address", function(event, Address){
                    $scope.Address = Address;
                    $scope.updateData($scope.Address);
                });
                $scope.updateData = function (Address) {
                    ...
                    $scope.elementGetValues = function () {
                        ...
                        console.log($scope.Address)
                        $scope.elementDataUrl1 = "http://localhost:2001/mbs/?m=0&t=e&r=";
                        $scope.elementDataUrl1 = $scope.elementDataUrl1 + $scope.Address;
                        //console.log($scope.elementDataUrl1);
                        $http.get($scope.elementDataUrl1).then(function (response) {
                            
                            $scope.elementValues1 = response.data;
                            //console.log($scope.elementValues1);
                            // Set the formatted data dataRaws for each element.
                            for (var respIdx = -1; respIdx < response.data.length; respIdx++) {
                                ...
                                if(respIdx == -1)
                                {
                                    ...
                                    var timestamp = response.data[respIdx].timestamp;
                                    var date = new Date(timestamp * 1000);
                                    var datevalues = ('0' + date.getDate()).slice(-2) + '-' + ('0' + (date.getMonth() + 1)).slice(-2) + '-' + date.getFullYear() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
                                    $scope.bdatetime = datevalues;
                                    $scope.data = response.data[respIdx].data;
                                    strData = "&label=" + $scope.bdatetime + "&value=" + response.data[respIdx].data + "&vLineColor=#666666&vLineThickness=2";    
                                }
                    ...
                    $scope.init = function(data){
                        ...
                        var date = new Date(timestamp * 1000);
                        var datevalues = ('0' + date.getDate()).slice(-2) + '-' + ('0' + (date.getMonth() + 1)).slice(-2) + '-' + date.getFullYear() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
                        return $scope.datavalue =  datevalues;
                    };
                   $scope.elementGetValues(); 
                }
            },
...
/*app.controller('Database_chart', ['$scope', '$http', function ($scope, $http) {
    $scope.$on('Address', function (event, Address) {
    // Build a request string used to GET the element data values from the server.
        $scope.elementDataUrl_1 = "http://localhost:2001/mbs/?m=0&t=e&r=";
        $scope.elementDataUrl_1 = $scope.elementDataUrl_1 + Address;
        $scope.database($scope.elementDataUrl_1);
         
    });
    $scope.database = function (elementDataUrl_1) {
        $http.get(elementDataUrl_1).then(function (response) {
            $scope.elementValues = response.data;
