...
void modbus_read_holding(duda_request_t *dr, modbus_t *ctx, int addr_start, int count)
{
    ...
    for (int i = 0; i < count; i++) {
		if (i == 0)
			...
			response->printf(dr, ",\n");
       response->printf(dr, "{\"addr\":%d, \"data\":%d}", addr_start + i,reg[i]);
	...
	for (int i = 0; i < list_len; i++) {
		...
		for (int j = 0; j < count; j++) {
			if (j != 0)
				...
				//set_data(addr + j, reg[reg_count2 + j]);
...
void database(duda_request_t *dr, modbus_t *ctx, uint16_t *list, int list_len)
{
...
void database_h(duda_request_t *dr, modbus_t *ctx, uint16_t *list, int list_len)
{
...
/*void database(){
    ...
    while (fgets(line, LINE_LEN, f) != NULL) {
        ...
        if (reg_num >= 0) {
			fprintf(stdout, "+   %ld = %ld\n", reg_num, reg_value);
			//create key in radis database

			if (simulate) {
				for (int i = 0; i < count; i++)
					reg[i] = shadow_reg[reg_num + i];
			}
			else {
				int rc = modbus_read_registers(ctx0, reg_num, count, reg);
			};
		    for (int i = 0; i < count; i++) {
				
		    	set_data(reg_num + i, reg[i]);
			    
			    //response->printf(dr, "{\"addr\":%d, \"data\":%d}", addr_start + i,reg[i]);
				shadow_reg[reg_num + i] = reg[i];
			};
...
void cb_query_string(duda_request_t *dr)
{
    ...
    case 'r':
    	database(dr, ctx, list, list_len);
        break;

    case 'e':
    	database_h(dr, ctx, list, list_len);
        break;
...
int duda_main()
{
	...
	if (simulate) {
		...
		ctx0 = modbus_new_rtu("/dev/ttyUSB0", 19200, 'N', 8, 1);
		...
		ctx1 = modbus_new_rtu("/dev/ttyUSB1", 19200, 'N', 8, 1);
